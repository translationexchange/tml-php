...
function stylesheet_tag($path) {
    echo '<link href="' . url_for('/assets/css/' .$path) . '" rel="stylesheet" />';
}
...
function javascript_tag($path) {
    ...
    echo '<script type="text/javascript" src="' . url_for('/assets/js/' . $path) . '"></script>';
}

function image_tag($path, $opts = array()) {
    echo '<img src="' . url_for('/assets/img/' . $path) . '" ' . ArrayUtils::toHTMLAttributes($opts) . ' >';
}
